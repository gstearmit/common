<?php
namespace common\components\Solr\Service;
...
class Balancer
{
	...
	{
		return ApacheSolrService::escape($value);
	}
	...
	{
		return ApacheSolrService::escapePhrase($value);
	}
	...
	{
		return ApacheSolrService::phrase($value);
	}
	...
	{
		if ($service instanceof ApacheSolrService)
		{
		...
		if (!$this->_currentReadService || !isset($this->_readableServices[$this->_currentReadService]) || $forceSelect)
		{
			...
			if (count($this->_readableServices))
			{
				...
				if (is_array($service))
				{
					...
					$service = new ApacheSolrService($service['host'], $service['port'], $service['path']);
					$this->_readableServices[$id] = $service;
				}
	 ...
	 * @throws Exception If an error occurs during the service call
	 */
	public function search($query, $offset = 0, $limit = 10, $params = array(), $method = ApacheSolrService::METHOD_GET)
	{
		$service = $this->_selectReadService();
